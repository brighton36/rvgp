...
class TestJournalParse < Minitest::Test
  SAMPLE_TAG_FORMATS_JOURNAL =<<~EOD
  ...
  SAMPLE_CURRENCY_DECLARATION_JOURNAL =<<~EOD
  2021-06-16 1100 HNL
  ...
  def test_journal_tag_parsing
    ...
    assert_equal 'Personal:Expenses:Home:LawnMaintenance', 
      posting0.transfers[0].account
    assert_equal '$ 100.00', posting0.transfers[0].commodity.to_s
    assert_equal 2, posting0.transfers[0].tags.length
    assert_equal ['intention: HomeMaintenance', 'property: 123GreenSt'], 
      posting0.transfers[0].tags.collect(&:to_s)

    # Posting 0, Transfer 1
    assert_equal 'Personal:Assets:Bankname:Checking', 
      posting0.transfers[1].account
    ...
    assert_equal 'Personal:Expenses:Dining', 
      posting1.transfers[0].account
    assert_equal '$ 50.00', posting1.transfers[0].commodity.to_s
    assert_equal ['wine: red', 'Pasta', 'Italian', 'Vegetarian'], 
      posting1.transfers[0].tags.collect(&:to_s)

    # Posting 1, Transfer 1
    assert_equal 'Personal:Assets:Bankname:Checking', 
      posting1.transfers[1].account
    ...
    assert_equal ['fabric: wool', 'width: 20', 'color: ancient white'], 
      posting3.transfers[0].tags.collect(&:to_s)
    ...
    assert_equal ['item: candy'], 
      posting4.transfers[0].tags.collect(&:to_s)
    ...
    assert_equal ['item: candy'], 
      posting5.transfers[0].tags.collect(&:to_s)
  ...
  def test_currency_purchase_parsing
    ...
    # Posting 0, Transfer 1
    assert_equal 'Personal:Expenses:Banking:Fees:IrlExchangeSlippage', 
      posting0.transfers[1].account
    ...
    assert_equal 's, d, t -> market(0, date, t)', 
      posting6.transfers[1].complex_commodity.left_lambda.to_s
