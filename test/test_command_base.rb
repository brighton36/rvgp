...
# Minitest class, used to test RRA::CommandBase
class TestCommandBase < Minitest::Test
  def test_remove_options_from_args
    ...
    assert_equal [{ stdout: true }, %w[target1 target2]],
                 RRA::CommandBase::Option.remove_options_from_args(options, ['--stdout', 'target1', 'target2'])
    assert_equal [{ stdout: true }, %w[target1 target2]],
                 RRA::CommandBase::Option.remove_options_from_args(options, ['target1', '--stdout', 'target2'])
    assert_equal [{ stdout: true }, %w[target1 target2]],
                 RRA::CommandBase::Option.remove_options_from_args(options, ['target1', 'target2', '--stdout'])

    options << RRA::CommandBase::Option.new(:date, :d, { has_value: 'DATE' })

    assert_equal [{ stdout: true, date: '2022-01-01' }, %w[target1 target2]],
                 RRA::CommandBase::Option.remove_options_from_args(
    ...
    assert_raises RRA::CommandBase::Option::UnexpectedEndOfArgs do
      RRA::CommandBase::Option.remove_options_from_args(options, ['--stdout', 'target1', 'target2', '--date'])
    end
  end
end
