...
# Minitest class, used to test RRA::Base::Command
class TestBaseCommand < Minitest::Test
  def test_remove_options_from_args
    options = [%i[all a], %i[list l], %i[stdout s], [:date, :d, { has_value: true }]].map do |args|
      RRA::Base::Command::Option.new(*args)
    end

    assert_equal [{ stdout: true }, %w[target1 target2]],
                 RRA::Base::Command::Option.remove_options_from_args(options, ['--stdout', 'target1', 'target2'])
    assert_equal [{ stdout: true }, %w[target1 target2]],
                 RRA::Base::Command::Option.remove_options_from_args(options, ['target1', '--stdout', 'target2'])
    assert_equal [{ stdout: true }, %w[target1 target2]],
                 RRA::Base::Command::Option.remove_options_from_args(options, ['target1', 'target2', '--stdout'])

    options << RRA::Base::Command::Option.new(:date, :d, { has_value: 'DATE' })

    assert_equal [{ stdout: true, date: '2022-01-01' }, %w[target1 target2]],
                 RRA::Base::Command::Option.remove_options_from_args(
    ...
    assert_raises RRA::Base::Command::Option::UnexpectedEndOfArgs do
      RRA::Base::Command::Option.remove_options_from_args(options, ['--stdout', 'target1', 'target2', '--date'])
    end
  end
end
