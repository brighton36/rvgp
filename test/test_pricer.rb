...
class TestPricer < Minitest::Test
  ...
  def test_price_format1
    ...
    assert_raises(RRA::Pricer::NoPriceError){ 
      price_on p, '1900-01-01', 'USD', 'FEQTX' }
    assert_raises(RRA::Pricer::NoPriceError){ 
      price_on p, '2004-06-20', '$', 'FEQTX' }
    assert_raises(RRA::Pricer::NoPriceError){ 
      price_on p, '2004-06-21', 'USD', 'FEQTX' }
    assert_equal "0.0444642063139173 FEQTX", 
      price_on(p, '2004-06-21 12:00:00', 'USD', 'FEQTX')
    assert_equal "0.0444642063139173 FEQTX", 
      price_on(p, '2004-06-21 14:14:59', '$', 'FEQTX')
    assert_equal "0.04395604395604396 FEQTX", 
      price_on(p, '2004-06-21 14:15:00', 'USD', 'FEQTX')
    assert_equal "0.04395604395604396 FEQTX", 
      price_on(p, '2004-06-21 14:15:01', '$', 'FEQTX')
    assert_equal "0.04395604395604396 FEQTX", 
      price_on(p, '2004-06-22', 'USD', 'FEQTX')
    assert_equal "0.04166666666666667 FEQTX", 
      price_on(p, '2004-06-25', '$', 'FEQTX')
    assert_equal "0.04166666666666667 FEQTX", 
      price_on(p, '2300-01-01', 'USD', 'FEQTX')
  end

  def test_price_format2
    p = RRA::Pricer.new TEST_PRICES_DB_FORMAT2
    ...
    assert_equal "1.109275 USD", 
      price_on(p, '2020-02-01', 'EUR', 'USD')
  ...
  def test_price_format1
    p = RRA::Pricer.new TEST_PRICES_LARGE_ASSETS

    # Make sure the database is parsed ok:
    assert_equal "$ 500000.00", 
      price_on(p, '2020-01-01 12:00:00', 'FLORIDAHOME', '$')
    assert_equal "$ 50000.00", 
      price_on(p, '2020-01-01 12:00:00', 'CORVETTE', '$')

    # I don't think anyone will ever do this, but, why not:
    assert_equal "0.000002 FLORIDAHOME", 
      price_on(p, '2020-01-01 12:00:00', '$', 'FLORIDAHOME')
    assert_equal "0.00002 CORVETTE", 
      price_on(p, '2020-01-01 12:00:00', '$', 'CORVETTE')
    ...
    assert_equal "0.000002 FLORIDAHOME",
      p.convert(at, dollar, 'FLORIDAHOME').to_s
    assert_equal "-0.000002 FLORIDAHOME",
      p.convert(at, minus_dollar, 'FLORIDAHOME').to_s
  end
