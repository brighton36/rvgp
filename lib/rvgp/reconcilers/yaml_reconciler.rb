...
module RVGP
  module Reconcilers
    ...
    # Currently, the following input formats are available. These forms support all of the
    # above features, and, may implement additional features outlined below.
    # - csv - this reconciler handles input files of type csv
    # - journal - this reconciler handles input files of type .journal (Pta accounting
    #   files)
    #
    ...
    #   If a string is provided, compares the
    ...
    #   This field matches any transaction
    ...
    class YamlReconciler < RVGP::Base::Reconciler
      ...
      # @!visibility private
      def postings
        @postings ||= case input_type
                      when :csv
                        postings_from_csv
                      when :journal
                        postings_from_journal
                      else
                        raise StandardError, "Unknown input format #{input_type.inspect}"
                      end.map do |source_posting|
      ...
      def input_type
        File.extname(input_file)[1..].to_sym
      end
      ...
      def postings_from_journal
        ret = RVGP::Journal::PtaFile.parse(File.read(input_file)).postings.map do |posting|
          ...
          # multiple RVGP::Reconcilers::YamlReconciler::Posting postings.
          posting.transfers[0...-1].map do |transfer|
            ...
            RVGP::Reconcilers::YamlReconciler::Posting.new posting.line_number,
                                                           date: posting.date,
