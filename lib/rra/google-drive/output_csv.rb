...
# This class is roughly, an kind of diagnostic alternative to OutputGoogleSheets,
# which implements the :csvdir option of the RRA::Commands::PublishGsheets command.
# Mostly, this object offers the methods that OutputGoogleSheets provides, and
...
class OutputCsv
  ...
  def sheet(sheet)
    ...
    CSV.open([destination.chomp('/'), '/', shortname, '.csv'].join, 'wb') do |csv|
      ([sheet.columns] + sheet.rows).each { |row| csv << row.map { |c| c.is_a?(Date) ? c.strftime('%m/%d/%Y') : c } }
    end
  end
end
