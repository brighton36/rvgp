...
module RRA
  ...
  class GridBase
    ...
    register_descendants RRA, :grids, accessors: {
      task_names: lambda { |registry|
        registry.names.map do |name|
          RRA.app.config.grid_years.map { |year| format 'grid:%<year>d-%<name>s', year: year, name: name.tr('_', '-') }
        end.flatten
      }
    }
    ...
    # This module can be included into children of RRA::GridBase, in order to add support
    # for multiple sheets, per year. This module contains class and instance helpers, that
    # will extend the GridBase syntax for year segmentation, into additional segments. These
    ...
    module HasMultipleSheets
      ...
      # at the time RRA::GridBase::HasMultipleSheets is included into a class.
      module ClassMethods
